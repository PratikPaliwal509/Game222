...
const TictactoeGameLogic = ({ onGameEnd }) => {
  ...
  useEffect(() => {
    const initializeGame = async () => {
      try {
        setError(null);
        const response = await axios.get(`https://game222.vercel.app/api/tictactoe/room/${roomId}`);
  ...
  useEffect(() => {
    const assignPlayers = async () => {
      ...
      try {
        const response = await axios.put(`https://game222.vercel.app/api/tictactoe/assignPlayers/${gameId}`, {
          player1: {
            playerId: playerId1,
            name: 'Player 1',
            symbol: 'X'
  ...
  const handleCellClick = async (index) => {
    ...
    try {
      ...
      const response = await axios.put(`https://game222.vercel.app/api/tictactoe/move/${gameId}`, {
        row,
        col,
        playerId: userId,
        symbol,
