  ...
  useEffect(() => {
    const initializeGame = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://game222.vercel.app/api/game/room/${roomId}`)
        if (response.data) {
        ...
        const createResponse = await axios.post('https://game222.vercel.app/api/game/create', gameData)
...
useEffect(() => {
  ...
  const handleChoice = async (choice) => {
    ...
    try {
      ...
      // Send move update
      const response = await axios.put(`https://game222.vercel.app/api/game/move/${gameId}`, {
        playerId: userId,
        move: choice,
      ...
      const updatedGame = await axios.get(`https://game222.vercel.app/api/game/room/${roomId}`)
  ...
  const evaluateRound = async (p1Choice, p2Choice) => {
    ...
    try {
      ...
      const currentGame = await axios.get(`https://game222.vercel.app/api/game/room/${roomId}`)
      ...
      await axios.put(`https://game222.vercel.app/api/game/round/${gameId}`, {
        round: updatedRound
      })
...
// Example usage:
        ...
        // Update room status
        await axios.put(`https://game222.vercel.app/api/room/${roomId}/status`, {
          status: 'finished',
          winner: gameWinner
        })
      } else {
...
async function updateGameWinner(roomId) {
  try {
    const response = await fetch(`https://game222.vercel.app/api/game/update-winner/${roomId}`, {
      method: 'PUT'
    });
